...
int execFifthTest()
{
    ...
    ft::vector<int>::const_iterator pointer = ftv.begin();
    int                     index = -1;
    ...
    try {
        ftv.at(index);
        printError("at: fifth test: exception doesn't throw", stv, ftv, 1);
        return 0;
    }
    catch (std::exception &e) {}

    if (pointer != ftv.begin())
    {
        printError("at: fifth test: reallocation of memory unnecessary", stv, ftv, 1);
        return 0;
    }
    return (1);
}
...
int execFourthTest()
{
    ...
    try {
        ftv.at(index);
        printError("at: fourth test: exception doesn't throw", stv, ftv, 1);
        return 0;
    }
    catch (std::exception &e) {}
...
int main()
{
    ...
    result += execFifthTest();
	if (result == 5)
		flag = 1;
	printResult("at", flag, timeFt, timeStd);
	return (1);
}
